...
const lessons: Lesson[] = [
  ...
  },
  {
    ...
    description: 'Working with data using pandas, numpy, and other data science libraries',
    content: 'Data handling is essential for modern Python applications. Learn to work with structured data, perform analysis, and manipulate datasets efficiently.',
    difficulty: 'advanced',
    ...
    examples: [
      {
        id: '28-1',
        title: 'Pandas Basics',
        description: 'Introduction to pandas for data manipulation',
        code: 'import pandas as pd\nimport numpy as np\n\n# Creating DataFrames\ndata = {\n    "Name": ["Alice", "Bob", "Charlie", "Diana"],\n    "Age": [25, 30, 35, 28],\n    "City": ["New York", "London", "Paris", "Tokyo"],\n    "Salary": [50000, 60000, 70000, 55000]\n}\n\ndf = pd.DataFrame(data)\nprint("DataFrame:")\nprint(df)\nprint(f"\\nShape: {df.shape}")\nprint(f"Columns: {list(df.columns)}")\nprint(f"Data types:\\n{df.dtypes}")\n\n# Basic operations\nprint(f"\\nFirst 2 rows:")\nprint(df.head(2))\n\nprint(f"\\nLast 2 rows:")\nprint(df.tail(2))\n\nprint(f"\\nBasic statistics:")\nprint(df.describe())\n\n# Selecting data\nprint(f"\\nSelecting columns:")\nprint(df["Name"])\nprint(df[["Name", "Age"]])\n\nprint(f"\\nFiltering data:")\nprint(df[df["Age"] > 30])\nprint(df[df["City"] == "London"])',
        explanation: 'Pandas provides DataFrame for structured data manipulation. Use head(), tail(), describe() for exploration and boolean indexing for filtering.'
      },
      {
        id: '28-2',
    ...
    exercises: [
      {
        ...
        expectedOutput: 'Should demonstrate data cleaning, analysis, and visualization',
        hints: ['Create realistic sample data', 'Handle missing values', 'Use groupby for analysis', 'Create multiple plot types']
      }
    ]
  },
  {
    ...
    description: 'Understanding Python memory management and optimization techniques',
    content: 'Python\'s memory management is automatic but understanding how it works helps optimize performance and avoid memory leaks.',
    difficulty: 'advanced',
    category: 'Python Core',
    examples: [
      {
        ...
        description: 'Understanding Python memory allocation and garbage collection',
        code: 'import sys\nimport gc\nimport psutil\nimport os\n\n# Memory usage of different data types\nprint("Memory usage of different data types:")\nprint(f"Integer: {{sys.getsizeof(42)}} bytes")\nprint(f"Float: {{sys.getsizeof(3.14)}} bytes")\nprint(f"String: {{sys.getsizeof(\'hello\')}} bytes")\nprint(f"List: {{sys.getsizeof([1, 2, 3])}} bytes")\nprint(f"Dictionary: {{sys.getsizeof(dict([(\'a\', 1), (\'b\', 2)]))}} bytes")\n\n# Memory usage of large objects\nlarge_list = list(range(10000))\n\nprint(f"\\nLarge list: {{sys.getsizeof(large_list)}} bytes")\n\n# Current process memory usage\nprocess = psutil.Process(os.getpid())\nprint(f"\\nCurrent memory usage: {{process.memory_info().rss / 1024 / 1024:.2f}} MB")\n\n# Garbage collection\nprint(f"\\nGarbage collection stats:")\nprint(f"Generation 0: {{gc.get_count()[0]}}")\nprint(f"Generation 1: {{gc.get_count()[1]}}")\nprint(f"Generation 2: {{gc.get_count()[2]}}")\n\n# Manual garbage collection\ngc.collect()\nprint(f"\\nAfter manual collection:")\nprint(f"Generation 0: {{gc.get_count()[0]}}")\nprint(f"Memory usage: {{process.memory_info().rss / 1024 / 1024:.2f}} MB")\n\n# Reference counting\na = [1, 2, 3]\nprint(f"\\nReference count of list: {{sys.getrefcount(a)}}")\nb = a\nprint(f"After assignment: {{sys.getrefcount(a)}}")\ndel b\nprint(f"After deletion: {{sys.getrefcount(a)}}")',
        explanation: 'Python uses reference counting and garbage collection. sys.getsizeof() shows object size, psutil tracks process memory, and gc manages garbage collection.'
      },
      {
        id: '29-2',
        title: 'Memory Optimization Techniques',
        description: 'Techniques to reduce memory usage and improve performance',
        code: 'import sys\nimport array\nimport numpy as np\nfrom collections import deque\n\n# Memory-efficient alternatives\nprint("Memory comparison of different data structures:")\n\n# Lists vs Arrays\nnumbers_list = [1, 2, 3, 4, 5] * 1000\nnumbers_array = array.array("i", [1, 2, 3, 4, 5] * 1000)\nnumbers_numpy = np.array([1, 2, 3, 4, 5] * 1000)\n\nprint(f"List: {sys.getsizeof(numbers_list)} bytes")\nprint(f"Array: {sys.getsizeof(numbers_array)} bytes")\nprint(f"NumPy array: {sys.getsizeof(numbers_numpy)} bytes")\n\n# Generators for memory efficiency\ndef fibonacci_list(n):\n    """Returns a list of Fibonacci numbers"""\n    result = []\n    a, b = 0, 1\n    for _ in range(n):\n        result.append(a)\n        a, b = b, a + b\n    return result\n\ndef fibonacci_generator(n):\n    """Generator for Fibonacci numbers"""\n    a, b = 0, 1\n    for _ in range(n):\n        yield a\n        a, b = b, a + b\n\n# Memory usage comparison\nfib_list = fibonacci_list(1000)\nfib_gen = fibonacci_generator(1000)\n\nprint(f"\\nFibonacci list: {sys.getsizeof(fib_list)} bytes")\nprint(f"Fibonacci generator: {sys.getsizeof(fib_gen)} bytes")\n\n# Using generators\nprint("\\nFirst 10 Fibonacci numbers from generator:")\nfor i, num in enumerate(fibonacci_generator(10)):\n    print(f"F({i}) = {num}")\n\n# Memory-efficient data structures\nprint(f"\\nDeque vs List for large collections:")\nlarge_list = list(range(10000))\nlarge_deque = deque(range(10000))\n\nprint(f"List: {sys.getsizeof(large_list)} bytes")\nprint(f"Deque: {sys.getsizeof(large_deque)} bytes")\n\n# Slots for memory optimization\nclass RegularClass:\n    def __init__(self, x, y):\n        self.x = x\n        self.y = y\n\nclass SlotsClass:\n    __slots__ = ["x", "y"]\n    \n    def __init__(self, x, y):\n        self.x = x\n        self.y = y\n\nregular_obj = RegularClass(1, 2)\nslots_obj = SlotsClass(1, 2)\n\nprint(f"\\nRegular class instance: {sys.getsizeof(regular_obj)} bytes")\nprint(f"Slots class instance: {sys.getsizeof(slots_obj)} bytes")',
        explanation: 'Use arrays for numeric data, generators for large sequences, deque for efficient collections, and __slots__ for memory-efficient classes.'
      },
      {
        id: '29-3',
        title: 'Profiling and Performance Analysis',
        description: 'Tools and techniques for profiling Python code',
        code: 'import time\nimport cProfile\nimport pstats\nimport io\nfrom functools import lru_cache\n\n# Simple timing\ndef slow_function(n):\n    """A slow function for demonstration"""\n    result = 0\n    for i in range(n):\n        result += i ** 2\n    return result\n\ndef fast_function(n):\n    """A faster version using sum and generator expression"""\n    return sum(i ** 2 for i in range(n))\n\n# Time comparison\nn = 100000\n\nstart_time = time.time()\nresult1 = slow_function(n)\ntime1 = time.time() - start_time\n\nstart_time = time.time()\nresult2 = fast_function(n)\ntime2 = time.time() - start_time\n\nprint(f"Slow function: {time1:.4f} seconds")\nprint(f"Fast function: {time2:.4f} seconds")\nprint(f"Speedup: {time1/time2:.1f}x")\nprint(f"Results match: {result1 == result2}")\n\n# Using cProfile\nprint(f"\\nProfiling slow function:")\nprofiler = cProfile.Profile()\nprofiler.enable()\nslow_function(10000)\nprofiler.disable()\n\n# Create stats object\ns = io.StringIO()\nstats = pstats.Stats(profiler, stream=s).sort_stats("cumulative")\nstats.print_stats(10)  # Print top 10 functions\nprint(s.getvalue())\n\n# Caching with lru_cache\n@lru_cache(maxsize=128)\ndef fibonacci_cached(n):\n    """Cached Fibonacci function"""\n    if n < 2:\n        return n\n    return fibonacci_cached(n-1) + fibonacci_cached(n-2)\n\ndef fibonacci_uncached(n):\n    """Uncached Fibonacci function"""\n    if n < 2:\n        return n\n    return fibonacci_uncached(n-1) + fibonacci_uncached(n-2)\n\n# Performance comparison\nn = 30\n\nstart_time = time.time()\nresult1 = fibonacci_cached(n)\ntime1 = time.time() - start_time\n\nstart_time = time.time()\nresult2 = fibonacci_uncached(n)\ntime2 = time.time() - start_time\n\nprint(f"\\nCached Fibonacci({n}): {time1:.4f} seconds")\nprint(f"Uncached Fibonacci({n}): {time2:.4f} seconds")\nprint(f"Speedup: {time2/time1:.1f}x")\nprint(f"Results match: {result1 == result2}")\n\n# Memory profiling with memory_profiler (if available)\ntry:\n    from memory_profiler import profile\n    \n    @profile\n    def memory_intensive_function():\n        """Function that uses a lot of memory"""\n        data = []\n        for i in range(10000):\n            data.append([i] * 1000)\n        return len(data)\n    \n    print(f"\\nMemory profiling (if memory_profiler is installed):")\n    memory_intensive_function()\nexcept ImportError:\n    print(f"\\nMemory profiler not available. Install with: pip install memory_profiler")',
        explanation: 'Use time.time() for simple timing, cProfile for detailed profiling, and lru_cache for function caching. Memory profiling requires the memory_profiler package.'
      }
    ],
    exercises: [
      {
        id: '29-ex1',
        title: 'Memory Optimization Challenge',
        description: 'Optimize a memory-intensive program',
        initialCode: 'import sys\n\n# Create a memory-intensive program\n# Your code here\n\n# Measure memory usage\n# Your code here\n\n# Optimize the program\n# Your code here\n\n# Compare memory usage\n# Your code here',
        expectedOutput: 'Should demonstrate memory optimization techniques',
        hints: ['Use generators instead of lists', 'Use appropriate data structures', 'Profile memory usage', 'Implement caching where appropriate']
      }
    ]
  },
  {
    ...
    description: 'Threading, multiprocessing, and asyncio for concurrent programming',
    content: 'Concurrency allows programs to handle multiple tasks simultaneously. Python provides threading, multiprocessing, and asyncio for different concurrency needs.',
    difficulty: 'advanced',
    category: 'Python Core',
    examples: [
      {
    ...
    exercises: [
      {
        ...
        description: 'Create a concurrent web scraper using asyncio',
        initialCode: 'import asyncio\nimport aiohttp\n\nasync def fetch_url(session, url):\n    """Fetch a single URL"""\n    # Your code here\n    pass\n\nasync def main():\n    """Main function to scrape multiple URLs"""\n    # Your code here\n    pass\n\n# Run the scraper\n# asyncio.run(main())',
        expectedOutput: 'Should demonstrate concurrent web scraping',
        hints: ['Use aiohttp for HTTP requests', 'Create session for connection pooling', 'Use asyncio.gather for concurrency', 'Handle errors gracefully']
      }
    ]
  }
]
